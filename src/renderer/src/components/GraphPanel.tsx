...
import { EnterReadEvent, ExitReadEvent, IPCTrace, WriteEvent, Event, OpenEvent, CloseEvent, FSEvent } from '@common/types';
import { toUniform } from '@common/utils';
import OverviewPanel from '@renderer/components/OverviewPanel';
...
interface GraphPanelProps {
    ...
    onGraphLoaded?: (ipcInstance: IPCTrace) => void
}
...
class GraphPanel extends Component<GraphPanelProps, GraphPanelState> {
    ...
    ipcInstance: IPCTrace | null = null
    ...
    removeProcess(processUUID: string, ipcInstance: IPCTrace) {
    ...
    removeFile(path: string, ipcInstance: IPCTrace) {
    ...
    exportInstance() {
        ...
        IPCTrace.exportToJSON(outIpcInstance)
    }
    ...
    loadInstance(name: string, content: string) {
        ...
        const ipcInstance = IPCTrace.createInstanceFromJSON(name, jsonInstance)
