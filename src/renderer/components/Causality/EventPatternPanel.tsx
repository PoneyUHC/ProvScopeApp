// @renderer/components/Causality/EventPatternPanel.tsx
...
export const EventPatternPanel: React.FC<EventPatternPanelProps> = ({
    ...
    const [codeInput, setCodeInput] = useState(
        initialCode ?? `event.eventType === "WriteEvent" && event.outputValues["content"] === "deadbeef" `
    );
    ...
    const addPattern = () => {
        try {
            ...
            const createdPattern = new EventPattern(trimmedName, codeInput);

            // Validate early if we can (so the user gets feedback immediately)
            const firstEvent = executionTrace.events[0];
            if (firstEvent) {
                createdPattern.matches(firstEvent as any);
            }
    ...
    const matchCountsByName = useMemo(() => {
        ...
        for (const eventPattern of patterns) {
            ...
            for (const candidateEvent of events) {
                try {
                    if (eventPattern.matches(candidateEvent as any) === true) {
                        matchingEventsCount += 1;
                    }
                } catch {
                    // Invalid predicate -> count as non-match (and keep UI usable)
                }
            }
    ...
    return (
        <div className="space-y-3 rounded-md border border-slate-200 bg-white p-4">
            ...
            <div className="text-sm text-slate-600">Predicate</div>
...
event.pid === 42
event.eventType === "OpenEvent"

Or statement-body:
if (event.pid > 0) return true;
return false;`}
            />
